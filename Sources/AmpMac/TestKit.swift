...
//
//  In-module test kit: `ampmac selftest` runs without Xcode; Tests/ bridges to XCTest with it.
...
enum TestKit {
    static var suites: [TestSuite] { [DSPSuite.suite, ChainSuite.suite, GuitarSuite.suite, PedalsSuite.suite, PresetsSuite.suite, DevicesSuite.suite, LevelSuite.suite, ChordmapSuite.suite, CLISuite.suite] }
    ...
    static func tempDir() -> URL {
        let u = FileManager.default.temporaryDirectory.appendingPathComponent("ampmac-test-\(UUID().uuidString)", isDirectory: true)
    ...
    static func run(filter: String? = nil) -> [Result] {
        ...
        for s in suites {
            for c in s.cases {
                ...
                let dir = tempDir()
                Prefs.defaults = UserDefaults(suiteName: "com.keithadler.ampmac.selftest")!
                Prefs.defaults.removePersistentDomain(forName: "com.keithadler.ampmac.selftest")
