...
final class Capture: @unchecked Sendable {
    ...
    func start(_ source: Source) throws {
        ...
        switch source {
        case .device(let uid):
            guard let d = Devices.find(uid: uid), d.inputs > 0, d.id != 0 else { throw EngineError(message: "That input is not here.") }
            device = d.id
        case .mac:
            device = try makeTap()
        }
        ...
        guard s == noErr else { stop(); throw EngineError(message: "Could not start listening (CoreAudio error \(s)).") }
        running = true
    }
    ...
    private func makeTap() throws -> AudioDeviceID {
        guard #available(macOS 14.2, *) else { throw EngineError(message: "Hearing the Mac's own sound needs macOS 14.2 or later. Pick an input instead.") }
        let desc = CATapDescription(stereoGlobalTapButExcludeProcesses: [Capture.selfProcessObject()])
        desc.uuid = UUID(); desc.name = "Amp for Mac Ear"; desc.isPrivate = true; desc.muteBehavior = CATapMuteBehavior.unmuted
        var tapID: AudioObjectID = 0
        let s = AudioHardwareCreateProcessTap(desc, &tapID)
        guard s == noErr, tapID != 0 else {
            throw EngineError(message: "macOS did not let Amp for Mac hear the system sound (error \(s)). Allow it under System Settings › Privacy & Security › Screen & System Audio Recording, then try again.")
        }
        tap = tapID
        ...
        guard let outUID = Devices.string(out, kAudioDevicePropertyDeviceUID) else { tearDownTap(); throw EngineError(message: "No output device to listen to.") }
        let agg: [String: Any] = [
            kAudioAggregateDeviceNameKey: "Amp for Mac Ear",
            kAudioAggregateDeviceUIDKey: "com.keithadler.ampmac.ear.\(getpid())",
        ...
        guard a == noErr, aggID != 0 else { tearDownTap(); throw EngineError(message: "Could not attach to the system sound (CoreAudio error \(a)).") }
        aggregate = aggID
        return aggID
    }
