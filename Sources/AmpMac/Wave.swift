...
enum Wave {
    ...
    static func read(_ url: URL) throws -> (samples: [Float], sampleRate: Double) {
        ...
        guard let buf = AVAudioPCMBuffer(pcmFormat: fmt, frameCapacity: AVAudioFrameCount(file.length)) else { throw EngineError(message: "Could not read \(url.lastPathComponent).") }
    ...
    static func write(_ url: URL, left: [Float], right: [Float], sampleRate: Double) throws {
        ...
        guard let buf = AVAudioPCMBuffer(pcmFormat: fmt, frameCapacity: AVAudioFrameCount(n)) else { throw EngineError(message: "Could not make a buffer.") }
