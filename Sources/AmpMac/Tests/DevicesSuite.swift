...
enum DevicesSuite {
    static var suite: TestSuite { TestSuite(name: "Devices", cases: [
        ...
        TestCase(name: "round trip adds two buffers and both latencies") { t in
            let solo = Devices.demo[0]
            let ms = Devices.roundTripMs(input: solo, output: solo, bufferFrames: 128)
            // (256 + 41 + 43) / 48000 = 7.08 ms
            t.check(abs(ms - 7.083) < 0.01, "\(ms) ms")
            t.check(Devices.roundTripMs(input: solo, output: solo, bufferFrames: 64) < ms, "smaller buffer, shorter trip")
        },
    ]) }
}
