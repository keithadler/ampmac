...
enum DSPSuite {
    ...
    static var suite: TestSuite { TestSuite(name: "DSP", cases: [
        ...
        TestCase(name: "tone shelves and peak move the right bands") { t in
            ...
            let crack = Biquad.peak(3500, gainDb: 5, q: 1.2, sampleRate: sr)
            t.check(abs(level(crack, at: 3500) - 5) < 0.3, "crack at 3.5 kHz: \(level(crack, at: 3500))")
            t.check(abs(level(crack, at: 300)) < 0.3, "crack leaves 300 Hz: \(level(crack, at: 300))")
