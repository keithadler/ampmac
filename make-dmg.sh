...
#   ./make-dmg.sh            -> build/Amp-for-Mac-<version>.dmg
...
DMG="build/Amp-for-Mac-$VERSION.dmg"
STAGE=$(mktemp -d)
...
Amp for Mac — how to install

1. Drag the "Amp for Mac" icon onto the "Applications" folder next to it.
...
4. Click the Apple menu (top-left) > System Settings > Privacy & Security.
   Scroll all the way down. Next to "Amp for Mac was blocked", click
...
https://github.com/keithadler/ampmac
TXT

rm -f "$DMG"
hdiutil create -volname "Amp for Mac" -srcfolder "$STAGE" -ov -format UDZO -quiet "$DMG"
